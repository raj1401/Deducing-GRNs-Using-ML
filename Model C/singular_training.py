...
# Different
def train_one_dataset(x_train,gamma,f_vec,time_steps,total_genes,genes_to_train,perm_exponents,batch_size,epochs,plot_gen_data,plot_f_vals,act_fn,check_data,num_iters):
    perm_list = acc_f.permutation_matrices(total_genes)
    ratio_topologies = np.zeros(len(perm_list))

    correct_exp = 2 * np.ones(genes_to_train) # Starting with an exponent of 2 for all genes
    for iter in range(num_iters):
        print(f"Training {iter+1}-th time")
        interactions, ms_error = train.train_network(x_train,gamma,f_vec,time_steps,total_genes,genes_to_train,correct_exp,batch_size,epochs,plot_gen_data,plot_f_vals,act_fn,check_data)
...
# Different
def train_all_sets(path_to_all_training_data,path_to_all_degradation,max_exponent=6,batch_size=16,epochs=50,plot_gen_data=False,plot_f_vals=False,act_fn='tanh',num_iters=100):
    ...
    perm_exponents = acc_f.generate_exp_combs(max_exp=max_exponent, genes_to_train=genes_to_train)
    ...
    ratios, topos = train_one_dataset(x_train,gamma,f_vec,time_steps,total_genes,genes_to_train,perm_exponents,batch_size,epochs,plot_gen_data,plot_f_vals,act_fn,x_train,num_iters)
    num_topos = len(topos)
...
if __name__ == '__main__':
    ...
    prob_matrix, topos = train_all_sets(train_data_list,deg_list,plot_gen_data=True,plot_f_vals=True,epochs=50,num_iters=1)
    print(prob_matrix)
